...
## D31 — Personal log: display-time override (no server round-trip, no re-rank), post-park prompt as a modal, verified_hours stored as OSM syntax (build-order step 13)

**Trigger:** Step 13's brief snippet (§7) defines `displayConfidence = personalConfidence(zoneId) ?? sourceConfidence(zoneId)` and lists six outcomes (`parked_ok | ticketed | sign_said_permit | unsafe | no_space | verified_hours`). Three implementation questions had to settle: (a) where the override is applied — server or client; (b) whether positive/negative outcomes should also *re-rank* the list; (c) how the post-park prompt surfaces given that the app is backgrounded during the drive.

**Decision:**
- **Override at display time only — client-side, never round-tripped through the backend.** The personal log lives in `app/src/lib/log/` (expo-sqlite, two tables `search` and `outcome` per §7) and is never synced. The override is applied per-zone in `MapResultsScreen` after `useWalkSearch`/`useTransitSearch`/`useZones` data lands: a `useOutcomes()` React Query hook materialises the entire outcome table once, then per-render we map each `ZoneFeature` through `applyPersonalOverride(p, ov)` to a new ZoneProperties with adjusted `confidence` / `eligible` / `zoneUnknown`. Rationale: the override is a *display* transform of server data, not new server input; backing it on the backend would mean syncing personal observations to a multi-tenant DB (which §7 explicitly refuses — "never synced").
- **Override semantics** (pure `personalOverride(outcome)` in `lib/log/personal.ts`):
  - `parked_ok` within 90 days → confidence **1.0**, `zoneUnknown` cleared, badge "Parked free here ×N".
  - `parked_ok` older than 90 days → confidence **0.8** (downgrades over time, but still positive).
  - `ticketed` → confidence **0**, `forceExcluded`, badge "Ticketed here" — rendered red regardless of server eligibility, never silently dropped from the list (the user wants to *see* they got ticketed there).
  - `verified_hours` → confidence **0.8**, `zoneUnknown` cleared, badge "Hours verified" + the OSM-syntax string surfaced in the detail view.
  - `sign_said_permit` → confidence **0.4** (data was wrong; treat like no-adapter), badge "Sign said permit".
  - `unsafe` / `no_space` → cosmetic badge only ("You marked unsafe" / "No space last time"); no confidence change because these aren't about the *rules*.
- **No re-rank.** The candidate order in the bottom sheet is the server score order; personal overrides change *how* each candidate is displayed (tier colour, dot count, badges) but never their position. Re-ranking would diverge from the server score order silently — confusing when a "parked OK before" zone bubbles to #1 even though it's 12 min away when there's a 4 min spot at #2. The override should add information, not reshuffle the ranking.
- **Post-park prompt as a native `<Modal>`, surfaced on `useFocusEffect` when there's a pending pick.** The flow: `recordSearch` writes one row when results land (keyed on a stable search-key so refetches don't duplicate); `recordPick(searchId, zoneId)` fires the moment the user taps Navigate; the prompt then appears the *next* time the screen regains focus (i.e. when the user comes back from Google/Apple Maps) IF the picked zone has no outcome younger than the search and the pick is within 6 h. Rejected: a second `@gorhom/bottom-sheet` stacked on the main one (gesture conflict + visual stacking jank). Rejected: an `AppState` watcher firing on backgrounded → foregrounded (fires too often, and on iOS the navigation stack already triggers `useFocusEffect` on return — fewer dependencies).
- **`verified_hours` stores raw OSM `opening_hours` syntax in `outcome.hours_observed`.** The verify flow's input is a free-text box (placeholder `Mo-Fr 08:30-18:30`) — we don't try to validate against `opening_hours.js` on the client (no dep there; the *server* uses it). On display, the verified hours show in the detail view; we don't yet re-run server inclusion against the verified string, so it doesn't change live eligibility — that bridge is left for §7's "developer-only sync action" feeding back into `waltham_forest_cpz_hours.json` etc.

**Why:** Doing the override server-side would force a personal-log sync — directly contradicting §7. Doing it client-side keeps the personal data on-device (private by default, no auth needed, no schema migration risk on a multi-tenant DB) and is exactly enough fidelity to drive display: the server's score order is still the source of truth for *which* candidates appear, and we only paint over them. The no-re-rank rule is deliberate — re-ranking would erode the user's mental model of "rank = how good the spot is", and `parked_ok` is already loud enough as a positive badge + bumped confidence + 3 dots. The `useFocusEffect` choice maps directly to the user's mental model: "I came back from Maps, the app is asking me what happened", with the bounded 6-h window preventing stale prompts after a long break.

**How to apply:** Step 13 = done. The log is wired from MapResults (both walk and transit pipelines record search + pick + apply overrides) and from the `Log` tab (`LogScreen` shows search history + outcome summary stats). Step 14 polish should add (a) the "developer-only sync action" that ports `verified_hours` back to the static-JSON adapters, (b) persistent React Query cache so the log survives a cold start, and (c) cold-start defaults that respect prior outcomes (e.g. open the map already showing the user's personal heat-map).

**Reconsider if:** Real use shows users *do* want re-ranking (e.g. parked_ok zones consistently get ignored because they sit at #7) — the override pipeline is one mapped-array away from changing sort order. Or if the post-park prompt is dismissed >50% of the time without an outcome (means the timing/wording is off — maybe surface in-Maps via a push instead). Or if `verified_hours` strings start getting re-typed by users at the same zone — means we should expose an edit flow rather than a fresh insert each time.

---

## D32 — Persisted RQ cache, sticky prefs (not destination), GPS as opt-in (build-order step 14)

**Trigger:** Step 14 ("polish: cold-start defaults, GPS-denial path, persistent React Query cache") bundles three small UX questions that the brief gestures at but doesn't pin down: (a) *what* survives a cold start, (b) *what* prefs are remembered, (c) *what happens when the user denies location*. The hypothesis-test pace of the project means each of these wants a defensible default, not endless option toggles.

**Decision:**
- **Persist *every* React Query response — including `/zones`, walk-search, transit-search, and the personal-log reads — to AsyncStorage**, via `@tanstack/react-query-persist-client` + `@tanstack/query-async-storage-persister`. One storage key (`parkfree.rq-v1`), one buster (`v1`), one maxAge (24 h), one throttle (1 s). Server queries get a stale shortcut to the last-known view on a flaky-tunnel cold start; the SQLite-backed log queries get a cache that survives the React Query in-memory cache between tab switches (the source of truth is still SQLite, so freshness is a no-op cost). We could *not* persist the log queries on the theory that they read from local SQLite anyway — but the cost of persisting them is tiny (a few KB), and skipping a class of queries selectively requires extra config that earns nothing.
- **Sticky prefs, but NOT a sticky destination.** `lib/prefs.ts` persists `{mode, timeMode, maxWalkMinutes, includeBus}` under `parkfree.prefs-v1` and the Search screen hydrates from it on mount (silently — disk defaults match in-memory defaults, no loading state needed). The destination is *not* persisted: re-opening the app onto last-trip's destination would be wrong as often as right (yesterday's drive is rarely today's drive), and the user typing a new destination is the universal-trigger that says "this is a fresh trip" — pre-filling would just train the user to ignore it. The personal-log search history (brief §7) already surfaces past destinations on the Log tab if the user actually wants to repeat one.
- **GPS is opt-in, never blocking.** We do not request location permission at app launch (the canonical iOS anti-pattern); we surface a "Use my current location" button in the Search screen and request the permission only when the user taps it. Failure modes are explicit: `permission_denied` → "Location off — search a destination manually, or enable Location for ParkFree in Settings"; `unavailable` (location services off, hardware error, timeout) → "Couldn't find your location — search a destination manually". The button never blocks search; both messages keep the manual-destination path one tap away.

**Why:** Persisting everything to a single AsyncStorage cell is cheap (the cache is comfortably under 1 MB), avoids the always-painful "this query was supposed to be persisted but isn't" surprise, and gets the tunnel-resilience win the brief asks for without per-query opt-in. Sticky prefs without a sticky destination splits the difference between "remember me" and "don't pretend you know what I want next" — *settings* are stable enough to amortise the setup cost across many trips; *destinations* aren't. Opt-in GPS respects the iOS reviewer-friendly pattern (only request when there's a clear, just-tapped reason), keeps the "GPS-denial path" the *primary* path (the search is fully usable without location), and makes the denial state a one-line hint instead of a dead-end modal.

**How to apply:** Step 14 = done; the build order is complete. Persistent cache: `App.tsx` uses `<PersistQueryClientProvider>` instead of `<QueryClientProvider>`. Prefs: hydrate via `loadPrefs()` in a `useEffect` on the Search screen mount, write via `savePrefs(...)` on submit (fire-and-forget — never block navigation on disk). GPS: call `getCurrentLocationPlace()` from a button handler; branch on `result.ok` for success vs the two failure reasons.

**Reconsider if:** Real-use telemetry shows the persisted cache is misleading (showing yesterday's "this street is free" when today it isn't, on the first paint before refetch lands) — narrow the maxAge from 24 h to something like 30 min and/or hide the data behind an "as of <when>" timestamp. Or if a user repeatedly searches the same destination across cold starts — promote the personal log's most-recent search to a pre-filled chip. Or if the GPS path turns out to be the *dominant* entry (every trip starts with "Use my current location") — auto-tap it on Search-screen focus when permissions are already granted, instead of waiting for the user.
