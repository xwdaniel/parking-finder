...
| 6 | Backend `GET /zones?bbox=&t=` — viewport-clipped GeoJSON with time-aware inclusion (§6.2); per-zone confidence incl. 0.4-tier for adapter-less areas | **Done** — `routes/zones.ts` + pure `zones/inclusion.ts` (`evaluateZone`) + `zones/opening-hours.ts` (`opening_hours.js`); per-borough confidence policy (Camden spatial 1.0 / WF tag-join 0.8 / Haringey·TH `cpz_area` 0.8, →0.6 `zoneUnknown` on positive in-CPZ evidence, →0.4 no-adapter); `active_cpz`/`red_route` excluded; server pinned `TZ=Europe/London`; returns *all* viewport zones with `eligible`/`confidence`/`reason` flags. `npm test` 55 green. Verified vs. live DB — Camden Cantelowes Tue 13:00 all excluded → Tue 21:00 21/23 eligible (the 2 still in CA-F(n) 08:30-23:00) → Sun all eligible; WF Fyfield-area Tue 13:00 20 excluded/27 `cpz_unknown`/17 free → Tue 17:00 only 3 excluded; Haringey 56/57 `in_cpz_area` 0.6 (see D25) |
| 7 | Search screen — Google Places autocomplete, mode toggle, time toggle, walk slider, bus toggle | **Done** — `app/src/screens/SearchScreen.tsx` + `components/DestinationInput.tsx` (Google Places when `extra.googlePlacesApiKey` set, else Apple on-device geocoder via `expo-location` — see D26) + `components/SegmentedToggle.tsx`; `Park near`/`Park + Tube` toggle, `Leave now`/`Arrive by` toggle with a compact `@react-native-community/datetimepicker`, 2–30 min walk slider (default 10, `@react-native-community/slider`), buses `Switch` (transit only, off), "Find free parking" gated on a chosen destination → `navigate('MapResults', {search})`. Also fixed an un-hoisted `expo-asset` so Metro bundles. `typecheck` clean, `expo-doctor` 17/17, `expo export` bundles (1179 modules) |
| 8 | Map + Results screen with real zones rendered, **walk mode only** — incl. per-zone "verify with signage" badge, borough-scoped warning banner + hatched tint (§4) | **Done** — `@rnmapbox/maps@~10.1.45` (SDK-52-compatible line); `MapResultsScreen.tsx` renders `<MapView>`(Light style) + `<ShapeSource>`/`<LineLayer>` colour-coded by a 5-tier `tierOf(props)` (best 1.0 / good 0.8 / unknown 0.6 verify-signage / low 0.4 / excluded), destination `<MarkerView>`, a top status strip ("N of M streets free now/by …", refetch spinner, "zoom in" when bbox too wide), the borough-scoped **warning banner**, and a bottom-left legend. `lib/api.ts` (`fetchZones` → `GET /zones`, typed mirror of the response) + `hooks/useZones.ts` (React Query, bbox snapped ~50 m, refetch on `onMapIdle`). Tokens via `app.config.ts` ← `app/.env` (`EXPO_PUBLIC_MAPBOX_ACCESS_TOKEN` → `extra.mapboxAccessToken`; `MAPBOX_DOWNLOAD_TOKEN` → `@rnmapbox/maps` plugin) — see D27; no token ⇒ a no-map fallback (destination + count + warning + street list). `typecheck` clean, `expo-doctor` 17/17, `expo export` bundles (1312 modules). **Hatched-area tint deferred** — needs an adapter-less borough's *boundary* polygon (not in `/zones`, and no adapter-less borough is ingested yet); the per-zone styling + warning banner (the safety-critical bits) are in place |
| 9 | `ST_ClosestPoint` walk scoring + bottom-sheet ranked list (collapsed/mid/expanded) | **Done** — `routes/search.ts` (`GET /search/walk?lat=&lng=&maxWalkMinutes=&t=`) runs the §5.1 walk-mode pipeline in one PostGIS round-trip: `ST_DWithin(zone.geom::geography, dest::geography, sliderMaxMinutes*80)` candidate prefilter → reuses pure `evaluateZone` for time-aware inclusion → `walkPoint = ST_ClosestPoint(zone.geom, dest)`, `walkMinutes = ST_Distance/80`, `score = walkScore*0.7 + confidence*0.3` (D5/D6); sorts desc, returns top 10; empty list ⇒ UI suggests widening the slider (never silently expand, D3). `MapResultsScreen` now layers a `@gorhom/bottom-sheet` (3 snap points 18/50/90%) over the existing time-aware map: collapsed = top-3 horizontal cards, mid = full ranked list with confidence dots (3/2/1, D13) + walk minutes, expanded = selected-zone detail (hours summary, verify-with-signage badge for `zoneUnknown`, walk-leg dashed line on the map, "Navigate · drive here" → Google Maps with Apple Maps fallback, D15). Bidirectional sync: tap a ranked zone polyline → fly camera to walk-point + expand sheet + select; tap a list row/card → same. `useWalkSearch` hook (React Query). Verified vs. live DB — Camden Square dest, Tue 13:00 → 0 / 87 candidates (all excluded by active Camden CPZs); Tue 21:00 → top-10 cpz_inactive 1.0 zones (Wrotham Rd / Agar Pl / …) scoring 0.80–0.95; WF dest Tue 17:00 → mix of cpz_inactive (0.8), free (0.8) and cpz_unknown (0.6) in score order. `npm test` 61 green; `typecheck` clean; `expo-doctor` 17/17; `expo export` bundles (1417 modules). See D28 |
| 10 | Backend TfL Journey integration + transit-mode scoring + disruption tiers | **Done** — new `tfl_stop` table + `npm run ingest:tfl-stops` (`sources/tfl-stops.ts`): TfL `/StopPoint/Type/NaptanMetroStation`+`NaptanRailStation`, filtered to tube/dlr/overground/elizabeth-line, 472 stations loaded. `tfl/journey.ts` wraps `/Journey/JourneyResults/{from}/to/{to}` (§5.3) — always coords, never stop IDs (§9); 5-min/5-min/50m/50m in-memory LRU cache (`(zonePoint, dest, time-bucket, timeIs, includeBus)`, brief §9); bounded-concurrency parallel runner. Pure `tfl/disruption.ts` classifies legs into `suspended | severe | minor | none` (§5.4, D12) — suspended filters the journey, severe ×0.7 the score. New `GET /search/transit?lat=&lng=&maxWalkMinutes=&t=&timeMode=&includeBus=` (`routes/search.ts`): one PostGIS round-trip (zones within 1.5 km of destination + K=3 nearest TfL stops per zone via `tfl_stop.geom <-> zone.geom LATERAL`) → drop excluded zones → fan out ≤ 25 zones × 3 stops parallel TfL calls (concurrency 6) → score `time*0.40 + walk*0.40 + confidence*0.20` per §5.2/D5 → dedupe per zone (keep best (zone,stop) pair) → top 10. 503 when TFL_APP_KEY missing (D29). Frontend: `useTransitSearch` hook, `MapResultsScreen` switches on `search.mode`; transit detail shows journey breakdown + Stop pin + dashed walk-to-stop leg; disruption badges per §5.4. Tests: 88 → 96 (transit-route validation + DB-down + key-missing 503; `disruption.test.ts` + `journey.test.ts` + `tfl-stops-transform.test.ts`). Backend typecheck clean; app typecheck clean; expo-doctor 17/17; expo export 1418 modules. See D29 |
| 11 | Journey summary view inside expanded bottom sheet | **Done** — backend `tfl/journey.ts` now extracts `routeOptions[0].lineIdentifier.id` + `.name` per leg into `NormalLeg.lineId` / `lineName` (walking legs stay null). Frontend `JourneySummary` sub-view in the expanded transit detail: header with worst-disruption pill, three-block timeline (Park & start · Arrive · Total), then a per-leg list — the implicit Drive leg first, then each TfL leg with a TfL-palette `lineBadge` (Northern black, Central red, Elizabeth purple, DLR teal, Overground orange, et al.) coloured by `lineId`, mode icon, from→to names, per-minute duration, and an inline disruption pill on the affected leg only (so the user sees *which* leg is delayed, not just the whole journey). Detail view now uses `BottomSheetScrollView` to stay swipeable when content overflows. See D30. `typecheck` clean; `expo-doctor` 17/17; `expo export` bundles (1418 modules). `npm test` 89 green |
| 12 | Google Maps / Apple Maps Navigate handoff | **Done** — `app/src/lib/navigate.ts`: pure `googleMapsUrl(lat,lng)` / `appleMapsUrl(lat,lng)` URL builders + a `navigateTo(lat,lng)` that tries Google Maps (`comgooglemaps://?daddr=…&directionsmode=driving`) and falls back to Apple Maps (`http://maps.apple.com/?daddr=…&dirflg=d` — Universal Link, no scheme registration needed) when Google Maps isn't installed or the call throws. iOS scheme registration: `app.json` → `ios.infoPlist.LSApplicationQueriesSchemes: ["comgooglemaps"]` so `Linking.canOpenURL` returns truthful results. Wired into both detail views (`MapResultsScreen.tsx`: walk `DetailView` line ~580, transit `TransitDetailView` line ~1080) and the no-map fallback list (so the feature works even without a Mapbox token). The drive leg is the *only* leg ParkFree hands off — walk legs at each end are on the user (per D15: no CarPlay-native UI, both maps apps carry their own CarPlay integration so the route shows on the car screen automatically when phone is connected). `typecheck` clean; `expo-doctor` 17/17; `expo export` bundles (1419 modules). See D15. |
| 13 | Personal log: `expo-sqlite` schema + post-park prompt + `verified_hours` flow + `displayConfidence` override | **Done** — `app/src/lib/log/`: `schema.ts` (idempotent CREATE TABLE for `search` + `outcome` per §7), `db.ts` (lazy `expo-sqlite` singleton w/ migration), `log.ts` (`recordSearch` / `recordPick` / `recordOutcome` / `listSearches` / `listOutcomes` / `getPendingPick`), pure `personal.ts` (`personalOverride(outcome) → {confidence, forceExcluded, clearZoneUnknown, badge}` + `applyPersonalOverride(p, ov)`). Hooks: `useOutcomes` / `useSearches` / `usePendingPick` / `useInvalidateLog` (React Query). UI: `components/OutcomePrompt.tsx` (native `<Modal>` post-park "How did it go?" — 6 outcomes + Skip; `verified_hours` opens an inline OSM-syntax text input). MapResultsScreen wiring: `recordSearch` fires once per (dest, mode, time, walk, bus) tuple when results land; `recordPick(searchId, zoneId)` fires on Navigate; personal overrides apply at render time to both `useZones` features and ranked candidates (re-painted tier colour + new badge + cleared `zoneUnknown`) — *no re-rank*, server score order stays authoritative (D31). `useFocusEffect` checks `getPendingPick()` (6 h window) on return from Maps and surfaces the OutcomePrompt. `LogScreen` replaces the placeholder: stat strip (searches / navigated / parked-OK / tickets) + per-search rows showing outcome (or "didn't pick" / "navigated — outcome pending"). `typecheck` clean; `expo-doctor` 17/17; `expo export` bundles (1436 modules). See D31. |
| 14 | Polish: cold-start defaults, GPS-denial path, persistent React Query cache | **Done** — three discrete polish items. **Persistent RQ cache**: `@tanstack/react-query-persist-client` + `@tanstack/query-async-storage-persister` + `@react-native-async-storage/async-storage`; `lib/persist.ts` wraps AsyncStorage as the persister (key `parkfree.rq-v1`, throttle 1 s, maxAge 24 h, buster `v1`); `App.tsx` swaps `<QueryClientProvider>` for `<PersistQueryClientProvider>` so `/zones` / `/search/walk` / `/search/transit` / log queries survive a cold start (network down, app killed, signal loss in a tunnel). **Cold-start defaults**: `lib/prefs.ts` (mode, timeMode, walk slider, includeBus) under `parkfree.prefs-v1`; `SearchScreen` hydrates from disk on mount (silently — in-memory defaults match disk defaults until load resolves), `savePrefs(...)` fire-and-forget on submit. Destination is NOT persisted (re-opening to last-trip's destination would be wrong as often as right). **GPS-denial path**: new `getCurrentLocationPlace()` in `lib/geocode.ts` reads → requests `expo-location` foreground perms only when needed, fetches `getCurrentPositionAsync`, reverse-geocodes for a tidy label, returns `{ok, place} \| {ok: false, reason: 'permission_denied' \| 'unavailable'}` — never throws. `SearchScreen` adds a "Use my current location" button below the destination input; on success the place pill is set, on denied → "Location off — search a destination manually, or enable Location for ParkFree in Settings", on unavailable → "Couldn't find your location — search a destination manually". Existing per-borough manual-search path stays the canonical fallback. `typecheck` clean; `expo-doctor` 17/17; `expo export` bundles (1453 modules). See D32. |
